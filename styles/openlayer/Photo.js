...
var ol_style_Photo = class olstylePhoto extends ol_style_RegularShape {
    constructor(options) {
        ...
        super({
            ...
            displacement: [
                ...
                (options.displacement[1] || 0) + sanchor,
            ],
            // No fill to create a hit detection Image (v5) or transparent (v6)
            fill: ol_style_RegularShape.prototype.render
                ? new ol_style_Fill({ color: [0, 0, 0, 0] })
                : null,
        })
        ...
        switch (this._kind) {
            ...
            case 'anchored': {
                context.roundRect(
                    this.sanchor_ / 2,
                    0,
                    width - this.sanchor_ - shadow,
                    height - this.sanchor_ - shadow,
                    strokeWidth
                )
                context.moveTo(
                    width / 2 - this.sanchor_ - shadow / 2,
                    height - this.sanchor_ - shadow
                )
                context.lineTo(
                    width / 2 + this.sanchor_ - shadow / 2,
    ...
    getImage(pixelratio) {
        ...
        var canvas = ol_style_RegularShape.prototype.getImage.call(
            this,
            pixelratio
        )
        if ((this._gethit || this.img_) && this._currentRatio === pixelratio)
            return canvas
        ...
        if (!this.getDisplacement) {
            ...
            a[0] =
                (canvas.width / pixelratio - this._shadow) / 2 - this._offset[0]
            if (this.sanchor_) {
                a[1] =
                    canvas.height / pixelratio - this._shadow - this._offset[1]
            } else {
                a[1] =
                    (canvas.height / pixelratio - this._shadow) / 2 -
                    this._offset[1]
            }
        }
